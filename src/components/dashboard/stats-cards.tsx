...
export function StatsCards({ trades }: StatsCardsProps) {
  const stats = useMemo(() => {
    ...
    const totalRr = trades
      .filter((t) => t.result === "Win" && t.rr)
      .reduce((acc, t) => acc + (t.rr || 0), 0);
  ...
  const StatCard = ({ title, value, unit, description, badge }: { title: string, value: string, unit?: string, description?: string, badge?: React.ReactNode }) => (
    <Card>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <StatCard title="Average RR" value={stats.avgRr.toFixed(2)} unit="R" description="On winning trades"/>
      <StatCard title="Net R" value={stats.netR.toFixed(2)} unit="R" description="Total R value across all trades"/>
    </div>
  );
}
